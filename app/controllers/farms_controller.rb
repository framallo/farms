class FarmsController < ApplicationController
  ...
  def index
    if params[:search]
      @farms = Farm.
        where("lower(name) like ? OR lower(address) like ?", "%#{params[:search].downcase}%", "%#{params[:search].downcase}%").
    ...
    def farm_params
      params.require(:farm).permit(:name, :description, :address, amenity_ids: [], activity_ids: [])
    end
